...
def get_read_end(read):
	if read.is_reverse:
		return read.reference_start+ 1
	return read.reference_end

def get_start_d(x,y):
	# get directional start
	x_start, y_start = get_read_start(x), get_read_start(y)
	x_end, y_end = get_read_end(x), get_read_end(y)

	if x.is_reverse != y.is_reverse:
		if abs(x_start - y_start) < abs(x_end - y_end):
			if x.is_reverse:
				return True, x_start - y_start, x_start,y_start
			else:
				return True, y_start - x_start, x_start, y_start
		else:
			if x.is_reverse:
				return False, y_end - x_end, x_start,y_start
			else:
				return False, x_end - x_end, x_start, y_start

	return False, np.inf, x_start, y_start
...
"""
def bam_to_dict(alignments, MAPQ):
...
def tabulate_start_positions_BE(bam=None, label=None, output_dir=None,  # inputs
	...
	ga = HTSeq.GenomicArray("auto", stranded=False, typecode="d")  # same as old ga_coverage
	...
	for read in reads_dict:
		...
		if len(read1_list) == 0 or len(read2_list) == 0:
			...
			for i in read1_list:
				for j in read2_list:
					first_read_position,second_read_position = None, None
					# nonmatching chromosomesq
					if i.reference_name == j.reference_name:
						flag, overlap_bp, first_read_position, second_read_position = get_start_d(i, j)

						# outie and within overlap distance
						if flag and (BEmodel_min_overlap<= overlap_bp <= BEmodel_max_overlap):
							if not i.flag & 0x400 and not j.flag & 0x400:
								noise_flag = False
								not_noise_count += 1
								...
								ga[HTSeq.GenomicPosition(i.reference_name, first_read_position)] += 1
								ga[HTSeq.GenomicPosition(i.reference_name, second_read_position)] += 1
								for pos in converted_pos_list:
									ga_converted[HTSeq.GenomicPosition(i.reference_name, pos)] += 1
								break


			if not second_read_position:
				first_read_position, second_read_position = get_read_start(i), get_read_start(j)
				if noise_flag:
					ga_noise[HTSeq.GenomicPosition(i.reference_name, first_read_position)]+=1
					ga_noise[HTSeq.GenomicPosition(i.reference_name, second_read_position)]+=1
				...
				PE_read_stats_list_of_dict.append({"read": read,
												   ...
												   "pos1": first_read_position,
												   "strand1": "-" if i.is_reverse else "+",
												   "pos2": second_read_position,
												   "strand2": "-" if j.is_reverse else "+",
												   "outie": flag,
	...
	return ga, ga_converted, ga_noise, PE_read_count,not_noise_count
...
def find_windows(ga_windows, window_size):
	...
	for iv, value in ga_windows.steps():
		if value:
			if iv.chrom != last.chrom or abs(iv.start - last.end) > window_size:
				last = iv
			else:
...
def output_alignments(narrow_ga,
	...
	for iv, value in ga_windows.steps():
		if value:
			...
			if alignment_found:
				if strand == '+':
					mm_start = iv.start - BEsearch_radius + int(start)
					mm_end = iv.start - BEsearch_radius + int(end)
				if strand == '-':
					mm_start = iv.end + BEsearch_radius - int(end)
					mm_end = iv.end + BEsearch_radius - int(start)
			...
			if alignment_found:
				...
				if tag not in reads_dict.keys():
					...
					matched_dict[tag] = [iv.chrom, target_start_absolute, target_end_absolute, name,target_strand_absolute,
										 ...
										 target_aligned,nuclease_converted_count, control_converted_count,
										 nuclease_noise_count,control_noise_count, iv.start, iv.end, iv, window_sequence]
				else:
					...
					matched_dict[tag] = [iv.chrom, target_start_absolute, target_end_absolute, name,target_strand_absolute,
										 ...
										 mismatches, insertions, deletions,
										 filename, target_name, target_name, full_name, target_sequence,
										 target_aligned, max(nuclease_converted_count,matched_dict[tag][17]), max(control_converted_count,matched_dict[tag][18]),
										 max(nuclease_noise_count,matched_dict[tag][19]), max(nuclease_noise_count,matched_dict[tag][20]),min(window_min[tag]), max(window_max[tag]), iv, window_sequence]
...
def alignSequences(targetsite_sequence, window_sequence, edist=8,bulges=1,max_score=7):
	...
	if chosen_alignment_m:
		...
		offtarget_sequence = offtarget_sequence_aligned = chosen_alignment_m.group()
		...
		if insertions + deletions >0:
			...
			target_aligned = targetsite_sequence
...
def compare(ref, bam, control, targetsite, BEsearch_radius,BEmodel_min_overlap, BEmodel_max_overlap, window_size, mapq_threshold,
	...
	nuclease_ga, nuclease_ga_converted, nuclease_ga_noise, total_nuclease_count,total_nuclease_not_noise_count = \
	...
	control_ga, control_ga_converted, control_ga_noise, total_control_count,total_control_not_noise_count = \
