...
## Found HemTools
## https://github.com/YichaoOU/HemTools/blob/master/bin/change_seq_off_target_bed.py
...
df[['#Chromosome', 'new_start', 'new_end', 'seq', 'Nuclease_Read_Count', 'Strand']].to_csv("test.bed", sep="\t",
...
for s, d in df.groupby('seq_length'):
    ...
    d[['#Chromosome', 'new_start', 'new_end', 'seq', 'Nuclease_Read_Count', 'Strand', 'Genomic Coordinate']].to_csv(out,
                                                                                                                    sep="\t",
                                                                                                                    header=False,
                                                                                                                    index=False)
