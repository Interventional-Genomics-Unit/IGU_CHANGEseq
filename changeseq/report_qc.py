...
def get_read_depth(pklfile):
    if check_file(pklfile):
        with open(pklfile, 'rb') as f:
            data = pickle.load(f)
    else:
        logger.info(f'Total read depth for {pklfile} could not be found for tpm normalization')
        logger.info('Running without normalizing')
    return data

def write_qc(qc_file,preprocessed_logfile=None,coverage_stat_file=None):
    ...
    if preprocessed_logfile is not None:
        ...
        if coverage_stat_file is not None:
